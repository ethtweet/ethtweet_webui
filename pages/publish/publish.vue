<template>
	<view class="publish_container">
		...
		<scroll-view scroll-x="true" class="images_list padd_h_48" v-if="imageUrl.length != 0">
			<view class="image_wrap" v-for="(item, index) in showImageUrl" :key="index">
	...
	export default {
		...
		methods:{
							...
							uni.uploadFile({
								...
								success: (uploadFileRes) => {
									if(uploadFileRes.statusCode == 200){
										...
										let json = JSON.parse(jsonStr);
										for(var j = 0; j < json.length; j++){
											let imgUrl = json[j].Hash +"?filename="+json[j].Name;
											that.imageUrl.push(imgUrl);
											let showImageUrl =  that.$api.ipfsResUrl() + json[j].Hash +"?filename="+json[j].Name;
											that.showImageUrl.push(showImageUrl);
			...
			deleteImg(index) {
				...
				that.showImageUrl.splice(index, 1);
			},
			cancel(){
				console.log('我取消掉了');
			},
...
.publish_container{
	...
	position: relative;
		...
		.publish_btn{
			background-image: url(../../static/publish/btn_bg.png);
			width: 198rpx;
			height: 62rpx;
